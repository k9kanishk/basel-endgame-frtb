...
import { TrendingUp, TrendingDown, AlertTriangle, BarChart3, Info, Target } from 'lucide-react'
import Papa from 'papaparse'
import {
  Card, CardContent, CardHeader, CardTitle,
  Button, Input, Label, Select, Option,
  Table, Thead, Tbody, Tr, Th, Td, Badge,
  Alert, AlertDescription, Dialog, DialogHeader, DialogTitle
} from '../components/ui/stubs'
import { fmt$, fmtPct } from '../utils/format'
import { mapAssetClassToRisk } from '../lib/basel_params'
import {
  computeSBM, K_to_RWA, applyOutputFloor,
  approxRatesGreeks, approxFXGreeks, approxEqGreeks
} from '../lib/frtb'
...
const ASSET_CLASSES = [
  ...
  'FX Risk',
  'Equity Risk - Large Cap',
  'Equity Risk - Small Cap'
]
...
export default function BaselEndgameRiskAnalysis() {
  ...
  const [newInstrument, setNewInstrument] = useState({
    ...
    maturity: 5,
    tradeType: 'Swap',
    assetClass: 'General Interest Rate Risk',
    desk: 'Rates',
  })
  ...
  function calculateRWA_SBM(instr: Instrument) {
    ...
    // CSV override (flat by instrument id)
    const ov = csvOverrides[instr.id]
    const greeks = {
      dv01: ov?.dv01 ?? instr.delta,
      vega: ov?.vega ?? instr.vega,
      curvature: ov?.curvature ?? instr.curvature,
    }
  ...
  const runAnalysis = () => {
    const calcs: RWACalculation[] = portfolio.map(instr => {
      ...
      return {
        ...
        preEndgameRWA: preRWA,
        postEndgameRWA: postRWA,
        deltaRWA: delta,
        percentageChange: preRWA === 0 ? 0 : (delta / preRWA) * 100,
        desk: instr.desk,
        audit,
      }
    })
    ...
    const summaries = Array.from(deskMap.values()).map(s => ({
      ...s,
      percentageChange: s.preEndgameTotal === 0 ? 0 : (s.deltaTotal / s.preEndgameTotal) * 100,
    }))
  ...
  function openAudit(calc: RWACalculation) {
    setSelected(calc)
    setAuditOpen(true)
  }
    ...
    Papa.parse<CSVRow>(f, {
      ...
      complete: res => {
        ...
        for (const row of res.data) {
          const id = row.instrument_id?.trim()
          if (!id) continue
      ...
      <div className="max-w-7xl mx-auto">
        <Card className="mb-6">
          <CardHeader>
            ...
            <p className="text-gray-600">
              FRTB-SA SBM engine with multi-risk-class, audit trail, CSV Greek overrides
            </p>
          </CardHeader>
        </Card>
        ...
        <div className="flex gap-2 mb-6">
          ...
          <Button
            ...
            disabled={deskSummaries.length === 0}
          >
            Hot-Spot Desks
          </Button>
        </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={newInstrument.notional}
                      onChange={e => setNewInstrument({ ...newInstrument, notional: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={newInstrument.maturity}
                      onChange={e => setNewInstrument({ ...newInstrument, maturity: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    <Label>Trade Type</Label>
                    <Select
                      value={newInstrument.tradeType}
                      onChange={v => setNewInstrument({ ...newInstrument, tradeType: v })}
                    >
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onChange={v => setNewInstrument({ ...newInstrument, assetClass: v })}
                    >
                  ...
                  <div>
                    ...
                    <Select
                      value={newInstrument.desk}
                      onChange={v => setNewInstrument({ ...newInstrument, desk: v })}
                    >
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600 mb-2">
                  Upload CSV with columns: <code>instrument_id,risk_class,bucket,type,value</code>. Types
                  are one of dv01, vega, curvature.
                </p>
              ...
              <CardContent>
                {portfolio.length === 0 ? (
                  ...
                  <Table>
                    <Thead>
                      <Tr>
                        <Th>Name</Th><Th>Notional</Th><Th>Trade</Th>
                        <Th>Asset Class</Th><Th>Desk</Th>
                        <Th>Delta</Th><Th>Vega</Th><Th>Curvature</Th>
                      </Tr>
                    </Thead>
                    <Tbody>
                      {portfolio.map(p => (
                ...
                {portfolio.length > 0 && (
                  <Button onClick={runAnalysis} className="mt-4">Run Risk Analysis</Button>
                )}
              </CardContent>
            </Card>
          </div>
        ...
        {viewMode === 'analysis' && (
          <div className="grid gap-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card>
                <CardHeader><CardTitle className="text-lg">Total Pre-Endgame RWA ($)</CardTitle></CardHeader>
                <CardContent><p className="text-2xl font-bold">{fmt$(totals.pre)}</p></CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle className="text-lg">Total Post-Endgame RWA ($)</CardTitle></CardHeader>
                <CardContent><p className="text-2xl font-bold">{fmt$(totals.post)}</p></CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle className="text-lg">Overall Change (%)</CardTitle></CardHeader>
                <CardContent>
                  <p className={'text-2xl font-bold flex items-center gap-2 ' + (totals.pct > 0 ? 'text-red-600' : 'text-green-600')}>
                    {totals.pct > 0 ? <TrendingUp /> : <TrendingDown />}{fmtPct(totals.pct)}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  <Thead>
                    <Tr><Th>Desk</Th><Th>Pre-Endgame RWA</Th><Th>Post-Endgame RWA</Th><Th>Change ($)</Th><Th>Change (%)</Th></Tr>
                  </Thead>
                  <Tbody>
                    {deskSummaries.map(s => (
                      <Tr key={s.desk}>
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  <Thead>
                    <Tr><Th>Instrument ID</Th><Th>Desk</Th><Th>Pre-Endgame</Th><Th>Post-Endgame</Th><Th>Change ($)</Th><Th>Change (%)</Th><Th>Audit</Th></Tr>
                  </Thead>
                  <Tbody>
            ...
            <Alert>
              <AlertDescription>
                <div className="flex items-center gap-2">
                  ...
                  These desks show the largest percentage increases in required capital under Basel III Endgame
                </div>
              </AlertDescription>
            </Alert>
            <Card>
              ...
              <CardContent>
                <Table>
                  <Thead>
                    <Tr><Th>Rank</Th><Th>Desk</Th><Th>Pre-Endgame</Th><Th>Post-Endgame</Th><Th>Capital Increase</Th><Th>Priority</Th></Tr>
                  </Thead>
                  <Tbody>
